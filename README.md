...
### 🏷️ タグオートコンプリート
プロンプト入力欄でタグを入力し始めると、Danbooru または e621 のタグ辞書から候補がポップアップ表示されます。↑↓キーで候補を選択し、Enter/Tabで確定できます。ソースの切り替え（Danbooru / e621）もポップアップ内のボタンで行えます。

### ⚖️ プロンプト強調値の調整（十字キーUI）
プロンプト入力欄でタグをタップ（またはテキストを選択）すると、そのタグの下に十字キー型のコントローラーが表示されます。
